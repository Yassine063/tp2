    ...
    public get router() : Router {
        ...
        */
        ...
        router.get('/',async (req:Request, res: Response) => {
            ...
            // const villes = req.locations;

            // if (typeof req.locations === 'string') {
              
            //     const json = await this._weatherService.readWeather(req.locations);
            //     res.json( json );
            // }
            
            const json = await this._weatherService.readWeathers(req.locations);
            res.json( json );
            // res.render('index', { ville: json });
        });

        router.get('/p-heure',async (req:Request, res: Response) => {
            ...
            if (typeof villes === 'string') {
                ...
                const json = await this._weatherService.readWeather(villes);
                res.json( json );
            }
